...
class Msupply_Paymentstatus_Adminhtml_Sales_OrderController extends Mage_Adminhtml_Sales_OrderController
{
    /**
    ...
    {	
		...
		if ($order = $this->_initOrder()) {
            try {
				...
				$__fields = array();
				$__fields['method'] = $paymentCode;
				$__where = $connection->quoteInto('parent_id =?', $orderId);
				$connection->update('zaybx_sales_flat_order_payment', $__fields, $__where);
				$connection->commit();
				}
				...
				$chequeDraweename = $data['chequedraweename'];
				$chequeDraweebank = $data['chequedraweebank'];
				$chequePannumber = $data['chequepannumber'];
				$connection = Mage::getSingleton('core/resource')->getConnection('core_write');
				$connection->beginTransaction();
				$__fields = array();
				$__fields['check_no'] = $chequeNo;
				$__fields['check_date'] = $chequeDate;
				$__fields['draweename'] = $chequeDraweename;
				$__fields['draweebank'] = $chequeDraweebank;
				$__fields['panno'] = $chequePannumber;
				$__where = $connection->quoteInto('parent_id =?', $orderId);
				$connection->update('zaybx_sales_flat_order_payment', $__fields, $__where);
				$connection->commit();
				...
				$ddNo = $data['ddno'];
				$ddDate = $data['dddate'];
				$ddDraweename = $data['dddraweename'];
				$ddDraweebank = $data['dddraweebank'];
				$ddPannumber = $data['ddpannumber'];
				$connection = Mage::getSingleton('core/resource')->getConnection('core_write');
				$connection->beginTransaction();
				$__fields = array();
				$__fields['dd_no'] = $ddNo;
				$__fields['dd_date'] = $ddDate;
				$__fields['draweenamedd'] = $ddDraweename;
				$__fields['draweebankdd'] = $ddDraweebank;
				$__fields['pannodd'] = $ddPannumber;
				$__where = $connection->quoteInto('parent_id =?', $orderId);
				$connection->update('zaybx_sales_flat_order_payment', $__fields, $__where);
				$connection->commit();
				...
				$paymentSatusCode = $data['paymentstatus'];
				if($paymentSatusCode)
				{	
				$connection1 = Mage::getSingleton('core/resource')->getConnection('core_write');
				$connection1->beginTransaction();
				$__fields = array();
				$__fields['payment_status'] = $paymentSatusCode;
				$__where = $connection1->quoteInto('entity_id =?', $orderId);
				$connection1->update('zaybx_sales_flat_order', $__fields, $__where);
				$connection1->commit();
				}
