<?php
	...
	function updateSeller($data)
	{
		...
		Mage::init();
		if(isset($data['sku']) && $data['sku'] != '' && isset($data['status']) && $data['status'] != '' && isset($data['price']) && $data['price'] != '' && isset($data['sellerCode']) && $data['sellerCode'] != '' && isset($data['inventory']) && $data['inventory'] != '')
		{
			...
			$proArr['data'] = null;	
			$proArr['message'] = "Seller code, sku, Price, Inventory and status are required";
		}
